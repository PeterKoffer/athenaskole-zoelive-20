...
Deno.serve(async (req) => {
  ...
  // Debug i serverens logs:
  console.log("[image-service] submitting to:", submitUrl, "prompt:", prompt, "w/h:", width, height);
  ...
  try {
    ...
    while (true) {
      ...
      if (status === "Ready") {
        ...
        if (!url || !/^https?:\/\//i.test(url)) return bad(`[endpoint=${submitUrl}] BFL poll: invalid result.sample URL`, 502);
