...
const game: AdventureGame = {
  ...
  async mount(el: HTMLElement, opts: GameStartOptions): Promise<GameHandle> {
    let score = 0; let paused = false; let actions = 0;
    el.innerHTML = `
      <div class="space-y-2">
    ...
    function add(points: number, msg: string) {
      score = Math.min(100, score + points); actions++; log.innerText = `${msg} → score ${score}`;
    ...
    const handle: GameHandle = {
      start(){}, pause(){ paused = true; }, resume(){ paused = false; }, destroy(){ el.innerHTML = ''; },
      async submit(){ return { ok:true, scoreDelta: score, data:{ actions } }; },
      on(){}, getState(){ return { score, actions, paused }; }
    };
    return handle;
  }
};
