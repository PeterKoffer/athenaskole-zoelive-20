...
export function onGameEvent(cb: (e: GameEvent) => void) {
  BUS.addEventListener('nelie:game', (ev: Event) => cb((ev as CustomEvent).detail));
}
...
export function postToServer(evt: GameEvent) {
  // TODO: send til Supabase edge endpoint (batch evt.)
  // fetch('/api/game-telemetry', { method:'POST', body: JSON.stringify(evt) });
}