...
const game: AdventureGame = {
  ...
  async mount(el: HTMLElement, opts: GameStartOptions): Promise<GameHandle> {
    let score = 0; el.innerHTML = `<div class="text-sm">Hold en 60 sek pitch: slogan + 2 argumenter.</div>
    ...
    return { start(){}, pause(){}, resume(){}, destroy(){ el.innerHTML=''; }, async submit(){ return { ok:true, scoreDelta: score, data:{ pitched: score===100 } }; }, on(){}, getState(){ return { score } } };
  }
};
export default game;