...
import KeywordEventModal from './KeywordEventModal';
...
export default function MultiLayerCalendar() {
  ...
  const filtered = events.filter(e =>
    visibleLayers.includes(e.layer) &&
    (e.layer !== 'internal' || userRole !== 'student') &&
    (e.layer !== 'keyword' || userRole !== 'student')
  );
  ...
  return (
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          {filtered.map(ev => (
            <div
              key={ev.id}
              className={`p-2 rounded border-l-4 ${layerColors[ev.layer]}`}
              onClick={() => {
                if (ev.layer === 'keyword' && userRole !== 'student') {
                  setActiveEvent(ev);
                }
              }}
            >
    ...
    {activeEvent && (
      <KeywordEventModal event={activeEvent} onClose={() => setActiveEvent(null)} />
    )}
    </>
  );
}
