...
import { Brain, Target, TrendingUp, BookOpen, Users, Award, Zap, BarChart3 } from "lucide-react";
...
const AILearningModule = ({
  ...
  const renderModeContent = () => {
    switch (activeMode) {
      case "curriculum":
        return <EnhancedCurriculumDashboard subject={subject} skillArea={skillArea} difficultyLevel={difficultyLevel} onBack={onBack} />;
      case "adaptive":
        return <EnhancedLearningSession subject={subject} skillArea={skillArea} difficultyLevel={difficultyLevel} onBack={onBack} />;
      case "focused":
        return <EnhancedLearningSession subject={subject} skillArea={skillArea} difficultyLevel={Math.max(1, difficultyLevel - 1)} onBack={onBack} />;
      default:
        return null;
    }
  };
      ...
      <Card className="bg-gray-900 border-gray-800">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            return <Card key={mode.id} className={`cursor-pointer transition-all duration-200 ${isActive ? 'bg-gray-700 border-lime-400' : 'bg-gray-800 border-gray-700 hover:border-gray-600'} ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`} onClick={() => handleModeSelect(mode.id)}>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex space-x-1">
                          {mode.recommended && <Badge variant="secondary" className="text-xs">
                              <Award className="w-3 h-3 mr-1" />
                              Recommended
                            </Badge>}
                          {mode.comingSoon && <Badge variant="outline" className="text-xs bg-slate-50">
                              Coming Soon
                            </Badge>}
                        </div>
                      </div>
                      ...
                      <div className="space-y-1">
                        {mode.features.slice(0, 3).map((feature, index) => <div key={index} className="flex items-center space-x-2">
                            <div className="w-1 h-1 bg-lime-400 rounded-full" />
                            <span className="text-gray-300 text-xs">{feature}</span>
                          </div>)}
                      </div>
                      
                      {isActive && <Button size="sm" className="w-full bg-lime-500 hover:bg-lime-600">
