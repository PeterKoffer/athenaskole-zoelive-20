...
const EnglishLearningTips = ({ skillArea = "english", currentActivity }: EnglishLearningTipsProps) => {
  ...
  const getEnglishTips = () => {
    ...
    return tipSets[skillKey as keyof typeof tipSets] || tipSets.english;
  };

  const tips = getEnglishTips();

  const handleSpeakTip = async (tip: string) => {
    if (isSpeaking) {
      stop();
    } else {
      await speakAsNelie(`Here's a helpful English learning tip: ${tip}`, true, 'english-tip');
    }
  };
  ...
  const handleSpeakAllTips = async () => {
    if (isSpeaking) {
      ...
      const allTipsText = `Here are some English learning tips: ${tips.join('. ')}`;
      await speakAsNelie(allTipsText, true, 'english-tips-all');
    }
  };
      ...
      <CardHeader>
        <CardTitle className="text-white flex items-center justify-between">
          ...
          <button
            ...
            title="Ask Nelie to read all tips"
          >
          ...
          {tips.map((tip, index) => (
            <li key={index} className="flex items-start justify-between group">
              ...
              <button
                ...
                className="ml-2 p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-amber-800/50 transition-all"
                title="Ask Nelie to read this tip"
              >
