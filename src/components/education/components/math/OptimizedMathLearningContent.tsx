...
const OptimizedMathLearningContent = ({ onBackToProgram }: OptimizedMathLearningContentProps) => {
  ...
  const handleNavigateToActivity = (index: number) => {
    console.log('[NAV BTN] Manual navigation requested to activity:', index);
    setManualActivityIndex(index); // ONLY update manualActivityIndex!
    // Do NOT call setCurrentActivityIndex here! Let the effect in useOptimizedLessonManager handle it.
  };

  // Log navigation and completion states for debugging
  useEffect(() => {
    console.log('[STATE] currentActivityIndex:', currentActivityIndex, 'isCurrentActivityCompleted:', isCurrentActivityCompleted, 'canNavigateBack:', canNavigateBack, 'canNavigateForward:', canNavigateForward);
  }, [currentActivityIndex, isCurrentActivityCompleted, canNavigateBack, canNavigateForward]);
